...
async def update_interaction_history(session_service, app_name, user_id, session_id, entry):
    """Add an entry to the interaction history as a session event.
    ...
    try:
        ...
        # Fetch the session object first
        ...
        if not session:
            print(f"Error updating interaction history: Session not found for user_id={user_id}, session_id={session_id}")
            return False

        # Append the event to the session using the correct method
